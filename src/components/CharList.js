...
class CharList extends Component {
    ...
    render() {
        ...
        const content = !(loading || error) ? char.map(obj => <Character key={obj.id} id={obj.id} onCharSelected={this.props.onCharSelected} character={obj}/>) : null;
...
const Character = ({character, onCharSelected, id}) => {
    const {name, thumbnail, noThumbnail} = character;
    return (
        <li className="characters__item">
            <button onClick={() => onCharSelected(id)} className="characters__link">
